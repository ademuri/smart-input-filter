...
/**
 * @brief Detects if a digital input is toggling (blinking) at a specific frequency.
 *
 * BlinkFilter monitors a digital signal and determines if it's alternating between
 * High and Low states within a specified time window (period) and tolerance.
 *
 * A blinking state is confirmed when a specified number of consecutive half-cycles
 * (transitions) fall within the expected duration (period / 2) ± tolerance.
 */
class BlinkFilter : public Filter<bool, bool> {
 public:
   ...
   * @param expectedPeriodMs The duration of a full High-Low cycle in milliseconds.
   * @param toleranceFraction The allowed deviation from the expected half-period.
   *                          Expressed as a fraction of 255 (e.g., 25 is ~10%).
   * @param lookbackPeriods The number of consecutive half-cycles that must be within
   *                        tolerance to set IsBlinking() to true.
   */
   ...
   *
   * @return true if the last `lookbackPeriods` half-cycles were within tolerance.
   */
